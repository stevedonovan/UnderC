...
//----------------------
{
  ...
  // no go!
  if (td_from == NO_MATCH && td_to == NO_MATCH) {
...
//-------------------------------------------
{
  ...
  FOR(i, n_sig) {
    ...
    FOR(j, n_arg) {
      //cout << *si << ',';
      if (mmatch[i][j] == NO_MATCH) {
        mmatch[i][j] = fmatch(*si, args[j]);
      }
      msum += mmatch[i][j];
      si++;
    }
...
// be converted
  ...
  try {
    ...
    Signature::set_fun_name("<unknown>");
  }
...
ambiguous_match:
  ...
  if (name == "[]") {
    return true;  // *hack 0.9.6 operator[]
  }
...
FunctionMatch::matches(PExprList el)
{
  ...
  if (el != NULL) { // then there are extra default arguments
    ...
    for(eli = el->begin();  eli != el->end();  ++eli, ++si) {
      ...
      if (mtype == NO_MATCH) {
        mtype = CONVERT_FROM_MATCH;  // _serious_ fiddle...
      }
...
// FUN_PTR_MATCH signals this, even though the match may be exact.
  ...
  FOR(j, n_arg) {
    ...
    t = *si++;
    if (t.is_pointer() && t.is_signature()) {
      mtype = FUN_PTR_MATCH;
    }
