...
public:
  ...
  {
    if (p == NULL) {
      return NULL;  // NULL maps onto NULL...
    }
    T1* pw = m_map[p];
    if (! pw) {
      pw = new T1(p);
...
{
  if (base == NULL) {
    return Parser::global().addr(m_entry->data);
  } else {
    return (char *)base + m_entry->data;
  }
}
...
XClass* XNTable::lookup_class(char *name, bool in_parent)
{
  ...
  if (pe->is_class()) {
    return wrap_XClass(pe->type.as_class());
  } else {
    return NULL;
  }
}
...
XTemplateFun* XNTable::lookup_template(char *name, bool in_p)
{
  ...
  if (pe->type.is_signature()) {
    ...
    if (pfe->get_template()) {
      ...
      return NULL;  // just a normal function
    }
  } else {
    return NULL;  // not a function at all
  }
}
...
void XNTable::get_functions(XFunctions& flist, int flags, char *pattern)
{
  ...
  flags |= FUNCTIONS;
  if (pattern == NULL) {
    m_table->list_entries(ls, flags);
  } else {
    m_table->search_entries(pattern, &ls, flags);
  }
  flist.clear();
...
XTemplateFun* XClass::get_template()
{
  TemplateInstance* ti = class_obj()->get_template();
  if (ti) {
    return new XTemplateFun(ti->get_template()->get_entry());
  } else {
    return NULL;
  }
}
...
// and now copy the buffer into the args array, backwards!
  ...
  if (returns_qword) {
    flags += Engine::RETURN_64;
  } else if (! rt.is_void()) {
    flags += Engine::RETURN_32;
  }
