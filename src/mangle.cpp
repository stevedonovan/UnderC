...
public:
  ...
  virtual char type_encode(Type t)
  {
    return 0;
  }
  virtual bool simple_type(Type t)
  {
    return false;
  }
  ...
  void out_ref_ptr(Type t)
  {
    ...
    if (t.is_ref_or_ptr()) {
      ...
      if (t.is_class() && ! m_dont_lookup_class) {
        outs(m_plain_obj);
      }
  }
  ...
  void do_signature(Signature *sig)
  {
    ...
    if (m_use_return_type) {
      ...
      if ((m_fn->is_constructor() || m_fn->is_destructor()) && !m_within_signature) {
        out('@');
      } else {
        // *fix 1.2.3 (Eric) Functions returning enums are encoded like those returning objects
        Type rtype = sig->return_type();
        if ((rtype.is_class() || rtype.is_enum()) && !rtype.is_ref_or_ptr()) {
          outs("?A");
        }
        out_type(rtype);
      }
  ...
  char *mangle(Function *pfn)
  {
    ...
    if (pc != NULL) { // is a method
      ...
      if (m_method_type_first) {
        out_method_type(access);
      }
      out_classname(pc);
      if (! m_method_type_first) {
        out_method_type(access);
...
// any arguments
    if (m_convention_first) {
      do_calling_convention();
    }
...
public:
  ...
  void out_fun_name(string fun_name) // MSMangler
  {
    ...
    if (m_fn->is_constructor()) {
      outs("?0");
    }  else if (m_fn->is_destructor()) {
      outs("?1");
    } else {
...
// *fix 1.2.4 class_context() is non-null if a function is a member (static!)
    if (! m_fn->class_context()) {
      Mangler::do_void_args();
    }
  }
  ...
  {
    if (!m_fn->is_static() && m_fn->is_const()) {
      out('C');
    }
  }
...
public:
    ...
    if (sig->begin() == sig->end()) {
      ...
      for (sigi = sig->begin(); sigi != sig->end(); ++sigi) {
        ...
        if (simple_type(m_t)) {
          ...
          if ((arg_idx = match_exact_arg(m_t)) != NOT_FOUND) {
            out_ref(arg_idx);
          } else {
  ...
  void out_fun_name(string fun_name) // GCC3Mangler
  {
    ...
    if (wuz_inside_class || wuz_inside_ns) {
      out('E');
    }
  }
    ...
    if (ali->m_pt == pt) { // a class or enum has been repeated!
      ...
      if (t.is_const()) {
        if (at.is_const()) {
          ...
          return idx;  // t has more qualifiers
        }
      }
