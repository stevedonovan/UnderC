...
token_lookup:
    ...
    if (pe && pe->is_typename()) {
      ...
      if (parens_follows || scope_follows) {
        ...
        } else if (tok == input.peek_next_token()) {
          c = yylex();   // shd be ::
          Parser::state.class_dcl = pe->type;
          class_entry = pe;
          return yylex();
        } else if (parens_follows) { // *hack 1.1.2 Function ptr declarations
          input.skip_whitespace();
          if (input.next_two("(*", false)) {
            return TYPENAME;
          }
  ...
  } else if (c < T_LAST) { //...must be a CONSTANT of some sort
    if (c == T_END) {
...
void uc_hash_cmd(char *s)
{
  ...
  if (rest) {
    input.insert_string(rest);  // push the rest into the token string!
  }
  input.user_cmd(cmd);
//input.discard_line();
...
PEntry Input::lookup_next_symbol()
{
  ...
  if (tok == TYPENAME_FUNCTION && input.next_two("::")) {
    if (!Parser::state.begin_scope(yylval.entry->type)) {
      return NULL;
    }
    tok = yylex();
