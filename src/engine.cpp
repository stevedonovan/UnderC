...
top:
        ...
        if (rmode) {
            rdata = ppcode->data;
            if (rmode == DIRECT) data = mDataSeg + rdata;
            else if (rmode == SREL)   data = baseSP + rdata;
            else data = mOP + rdata;
        }
...
call_the_function:
        ...
        case CALLN:  // native code interface
            ...
            mSP += nargs;
            if (pfb->flags & DC_QWORD) push2(*(double *)buff);
...
JMP_case:
        case JMP:
            ...
            break;
        case JZ:
            if(!pop()) goto JMP_case;
            break;
        case JNZ:
            if(pop())  goto JMP_case;
            break;
        case JZND:
            if(!tos()) goto JMP_case;
            else drop();
            break;
        case JNZND:
            if(tos())  goto JMP_case;
...
void  exec_message(char *msg, bool was_error, bool dont_unwind_error) {
    if (fb == START_FB) {
        cerr << msg << std::endl << "popped!!" << std::endl;
        return;
    }
    int ip_offs;
    string fname;
    ...
    try {
        do {
            ...
            if (back_trace && ppcode != NULL) {
                if (have_info) cerr << li.file << ' ' << li.line << ": " << fname << std::endl;
                else cerr << "(" << ip_offs << ") " << fname << std::endl;
                if (++kount > 50) {
        ...
        for(;;) {
            if (! start_exec()) {
                ...
                if (flags & ARGS_PASSED) {
                    ...
                    if (flags & RETURN_32) xargs->ret1 = pop();
                    else if (flags & RETURN_64) xargs->ret2 = pop2();
                } else mSP = old_mSP;
                return OK;
            }
        }
