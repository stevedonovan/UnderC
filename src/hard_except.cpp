...
void trans_func( unsigned int u, _EXCEPTION_POINTERS* pExp )
{
  ...
  _fpreset();
  if(u == STATUS_ACCESS_VIOLATION) {
    throw AccessViolation();
  } else if(u == STATUS_FLOAT_DIVIDE_BY_ZERO) {
    throw FloatDivByZero();
  } else if(u == STATUS_FLOAT_OVERFLOW) {
    throw FloatOverflow();
  } else if(u == STATUS_INTEGER_DIVIDE_BY_ZERO) {
    throw IntDivByZero();
  } else if(u == STATUS_INTEGER_OVERFLOW) {
    throw IntOverflow();
  } else {
