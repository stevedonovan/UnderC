...
// *fix 1.2.7 wasn't outputing 'operator' in front of '()'
  ...
  if (pc != NULL) {
    ...
    if (! s_full_names && pc->is_template()) {
      classn = pc->get_template()->name();
    }
    if (s_ctor) {
      na = classn;
    } else if (s_dtor) {
...
// remember to keep the parent updated!
  if (m_parent->type() == FUNCTION) {
    m_parent->alloc(sz, NULL);
  }
...
void LocalContext::add_line_no(const string& file, int line) // override
{
  LineInfo li;
  if (function() == NULL) {
    return;  // local context in interactive mode
  }
  li.file = file;
  li.cntxt = this;
  li.line = line;
...
void FunctionContext::initialize()
{
  ...
  FBlock *fb = fun_block();
  if (fb->pstart) {
    delete fb->pstart;  // may be previously defined!
  }
  fb->pstart = 0;
...
// ensure that all returns jump to it first!
  if (ret_lbl && code.last_pi()->opcode == UNWIND) {
    ret_lbl->here(-1);
  }
...
void ConstructorContext::initialize()
{
  ...
  if (!function()->builtin()) {
    function()->class_context()->auto_construct(false);
  }
}
...
{
  if (!function()->builtin()) {
    function()->class_context()->auto_destruct(false);
  }
...
{
  return m_sig->return_type();
}

LineNumbersB *    Function::line_nos()
{
  return m_line_nos;
}

PEntry Function::return_object()
{
  return m_ret_obj;
}

void Function::return_object(PEntry pe)
{
  m_ret_obj = pe;
}

bool Function::stdarg()
{
  return signature()->stdarg();
}

void Function::stdarg(bool stdarg)
{
  signature()->stdarg(stdarg);
}
...
void Function::builtin(uchar b)
{
  m_builtin = b;
}

bool Function::is_cdecl()
{
  return m_builtin == CDECL;
}

bool Function::stdmethod()
{
  return m_builtin == STDMETHOD;
}

ImportScheme *Function::import_scheme()
{
  return m_import_scheme;
}

void Function::import_scheme(ImportScheme *is)
{
  m_import_scheme = is;
}

void Function::adjust_return_type(Type t)
{
  m_sig->adjust_return_type(t);
}

void Function::fun_block(FBlock *fblk)
{
  m_fun_block = fblk;
}

FBlock *Function::fun_block()
{
  return m_fun_block;
}

FunctionEntry *Function::fun_entry()
{
  return m_fun_entry;
}

bool Function::has_default_args()
{
  return m_default_args != NULL;
}

Class *Function::class_context()
{
  return signature()->class_ptr();
}

bool Function::is_const()
{
  return signature()->is_const();
}
...
bool Function::export_as_cdecl()
{
  return m_cdecl;
}

void Function::export_as_cdecl(bool yesno)
{
  m_cdecl = yesno;
}

int Function::slot_id()
{
  return m_slot_id;
}
...
TemplateInstance *Function::get_template()
{
  return m_templ_info;
}

void Function::set_template(TemplateInstance *pti)
{
  m_templ_info = pti;
}
...
//if (!is_plain() && pc) fail("must be within class definition");
  if (is_destructor()) {
    if(signature()->size() > 0) {
      fail("destructor cannot be passed parameters");
    }
    pc->destructor(this);
  } else if (is_constructor()) {
    pc->add_constructor(this, no_implicit_type_conversion);
  }
}
...
void Function::set_default_args(PExprList pel)
...
PExprList Function::complete_arg_list(PExprList args)
{
  if (!m_default_args) {
    return NULL;
  }
...
string Function::as_str()
{
  ...
  std::ostrstream out(buffer, MAX_FUN_NAME);
  dump(out);
  out << std::ends;
  return buffer;
}

Function *Function::from_fun_block(FBlock *pfb)
{
...
void FunctionEntry::dump (std::ostream& os)
{
  iterator ife;
  int idx = 1;
...
Function *FunctionEntry::nth_fun(int idx)
{
  iterator fei = begin();
  Function *fn;
  if (idx == 0) {
...
Function *FunctionEntry::fun_from_fblock(FBlock *pfb)
{
  for(iterator fei = begin(); fei != end(); ++fei)
    if ((*fei)->fun_block() == pfb) {
      return *fei;
    }
  return NULL;
}
...
// *fix 1.2.2b (Eric) Functions w/ no args can be incorrectly matched
  ...
  if (size() != sig.size() || !class_ptr() != !sig.class_ptr()) {
    return false;
  }
...
Function *FunctionEntry::simple_match(Signature *sig)
{
  iterator ife;
  for (ife = begin(); ife != end(); ++ife) {
    _fn = *ife;
...
Function *FunctionEntry::full_match(Signature *)
{
