...
// only work for simple types at the mo.
  ...
  if (t == t_null) {
    m_type = t_null;  // that is, unbound us (NB for us to _reuse_ this)
    ...
    if (Parser::debug.verbose) {
      std::cout << "*bound (const) " << name() << ' ' << *(int *)pe->global_ptr() << std::endl;
    }
  } else {
    if (! unbound() && t != m_type) {
      return false;
    ...
    if (Parser::debug.verbose) {
      std::cout << "*bound " << name() << ' ' << t << std::endl;
    }
  }
  return true;
}
...
string DummyType::value_as_string()
{
  ...
  if (name() != "") {
    ...
    if (entry() == NULL) {
      ...
      s += type().value_as_string(entry()->global_ptr());
    }
  }
  return s;
}
...
// resolving the actual type, given an expression like Type <int,20> etc.
  ...
  if(pte->match(*ptl)) {
    ...
    error("Cannot match this template");
  }
  return t_null;
}
...
void Template::do_template_header(TypeList *ptl)
{
  ...
  FORALL(tli, *ptl) {
    ...
    PEntry pe;
    if (is_dummy) {
      pe = state.add_typedef(type, name);
    } else {
      pe = state.add_variable(type, name, NULL, None);
    }
...
// *hack 1.1.4 lexical tie-in
  ...
  try {
    ...
    if (sig->class_ptr() != NULL && was_outside_class) {
      cntxt = sig->class_ptr();
      if (PClass(cntxt)->get_template() == NULL) {
        fail("must be a class template here");
      }
      was_method = true;
    } else {
      cntxt = &Parser::state.context();
    }
    ...
    if (! was_method) {
      ...
      if (!ptmpl->is_method()) {
        FORALL(tlip, formal_parms)  {
          int times_present = 0;
          FORALL(tlia, formal_args) if (type_contains(*tlia, *tlip)) {
            times_present++;
          }
          all_present = all_present && times_present > 0;
        }
        if (!all_present) {
          fail("All formal template type parms must be present at least once");
          return;
        }
      }
    }
...
// *fix 0.9.8 Doing class template specializations properly
  ...
  if (pe && pe->type != t_template_type) {
    error("Not a template class " + Parser::quotes(name));
    return;
  }
  if (ptl != NULL) { // formal_args will only be non-trivial if this is a specialization;
    if (pe == NULL) {
      error("Can only specialize a class template if it already exists");
      return;
    }
    formal_args = *ptl;
  } else {
...
// (b) t1 is a template containing t2, e.g. list<T,A>&
{
  if (t1.is_dummy() && t2.is_dummy()) {
    ...
    if (d1 == d2 || d1 && d2 && d1->name() == d2->name()) {
      return true;  // case (a)
    }
    if (t1.is_class()) {
...
void massage_type_list(TypeList& tl)
{
  ...
  FORALL(tli, tl) {
    Type t = *tli;
    if (t.is_const() && ! t.is_ref_or_ptr()) {
      t.strip_const();  // e.g. 'const int' -> 'int'
    }
// 'variable references'! (see Expressions::entry_op())
    else if (t.is_reference()) {
      t.strip_reference();  //t.is_variable()
    }
...
// in general stuff will already be written into the buffer; may (or not) include the ':' or '{'
  ...
  if (Parser::debug.verbose) {
    std::cout << "*buff: " << mIBuffer << std::endl;
  }
...
Template *Template::as_template(Type t)
{
    ...
    // not just the particular instance!
  {
    ...
    return NULL;  //*NOTE* until we can sort this out!
  }
}
...
int Template::match(const TypeList& tl)
{
  TLCI tlif, tlia;
  if (tl == m_formal_args) {
    return 999;  // *fix 1.0.0 full specialization
  }
  int effort = 0;
  for(tlif = m_formal_args.begin(), tlia = tl.begin(); tlia != tl.end(); ++tlif, ++tlia) {
    ...
    // or a template class.
    if (!tf.is_bare()) {
      return err("cannot match");
    }
    if (ta.is_const() && ta.is_ref_or_ptr()) {
      return err("parameter is not const");
    }
    effort++;
      ...
      if(ta.is_template_class()) {
        ...
        if (titf->get_template() == tita->get_template()) {
          ...
          for(tlif = tlf.begin(), tlia = tla.begin(); tlif != tlf.end(); tlif++, tlia++) {
            if (!tlif->as_dummy()->bind_to(*tlia)) {
              return err("already bound to a diff. type");
            }
          }
        } else {
          return 0;  // no way we can match different templates!
        }
      } else {
        return 0;  // cannot match template w/ a non-template class!
      }
    } else {
      if (! tf.as_dummy()->bind_to(ta)) {
...
{
  if (m_template->is_class()) {
    return Type((Class *)data());
  } else {
    return Type(((Function *)data())->signature());
  }
...
{
  if (fn->get_template()) {
    return true;  // fine - already instantiated!
  }
...
int TemplateEntry::simple_match(const TypeList& tl, bool use_args)
{
  ...
  FORALL(tili, m_instances) {
    ...
    if (Parser::debug.verbose) {
      std::cout << "*cmp " << tl << ' ' << tlinst << std::endl;
    }
    if (tl == tlinst) {
      return i;
    }
...
// returns the number of _instantiated_ instances
{
  TemplateInstanceList::iterator tili;
  int i = 0;
  FORALL(tili, m_instances) if ((*tili)->instantiated()) {
    i++;
  }
  return i;
}
...
{
  if (m_method_entries == NULL) {
    m_method_entries = new EntryList();
  }
...
// Build up a candidate list of templates which could theoretically match this list
  ...
  if ((*teli)->formal_args().size() == no_args) {
    tel.push_back(*teli);
  }
  if (tel.size() == 0) {
    return err("Wrong number of type arguments");
  }
