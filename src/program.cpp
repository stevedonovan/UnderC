  ...
  int execute()
  {
    ...
    while (run_main_function(flags, retval) == HALTED) {
      if (Errors::check_output()) {
        IDE::breakpoint();  // Inform the IDE, if it's a remote command
      }
      main_state(HALTED);
...
// *fix 1.1.4 We push local context while debugging; make sure it's popped before restarting...
  ...
  if (nargs == 0) {
    ...
    error("Wrong no of args for main()");
    return 2;
  }
...
{
#ifdef _WCON
  if (get_main_state() == FINISHED) {
    Program::stop_main(false);
  }
#endif
