...
PEntry symbol_lookup(const string& name)
{
    ...
    PClass this_class;
    if (is_class(context)) this_class = (Class *)context;
    else if (function) this_class = function->class_context();
    ...
    if (base_access != Public) {
        if (base_access == Private) access = Private;
        else if (base_access == Protected && access == Public) access = Protected;
    }
...
Function *ParserState::start_function(Type t, const string& name,bool init_context, int ftype)
{
    ...
    try {
        ...
        Table *cntxt = &context();
        if (fn->is_constructor()) fcntxt = new ConstructorContext(cntxt,fn);
        else if (fn->is_destructor())  fcntxt = new DestructorContext(cntxt,fn);
