...
Signature *unique_signature(Signature *sig)
{
  ...
  for(; is != ie; ++is) {
    ...
    if (s->return_type() == sig->return_type() && s->match(*sig, true)) {
      return *is;
    }
  }
  return sig;
...
// hmm...are bools considered a kind of integer?
  if (t != TT_VOID && t != TT_FLOAT && t != TT_BOOL) {
    m_int = 1;
  } else {
    m_int = 0;
  }
...
void Type::complex_init(TypeEnum t, void *type_data)
{
  *((long *)this) = 0;  //*SJD* Nasty but effective!
  if (t == TT_CLASS || t == TT_NAMESPACE) {
    m_class = 1;
    m_extra = index_into(_classes_, (Class *)type_data);
  ...
  } else if (t == TT_SIGNATURE) {
// *change 1.2.2 (Eric) Always try to keep signatures unique
    m_signature = 1;
    m_extra = index_into(_signatures_, find_signature(type_data));
  } else if (t == TT_ENUM || t == TT_DUMMY) {
    if (t == TT_DUMMY) {
      m_short = 1;
    }
    m_enum = 1;
...
bool Type::is_unbound() const
{
  ...
  if (!is_dummy()) {
    return true;  // everything else has a definite type value
  }
...
int      Type::inherits_from(Type t) const
{
  if (is_class() && t.is_class()) {
    return as_class()->inherits_from(t.as_class());
  } else {
    return 0;
  }
}
...
NamedTable* nested_context(PEntry pe)
{
  ...
  if (type == IS_NAMESPACE || type == IS_STRUCT) {
    if (Parser::debug.use_typedef_names && pe->context == Parser::std_namespace()
        && Parser::global().is_injected((Namespace*)pe->context)) {
      return NULL;
    } else {
      return (NamedTable*) pe->context;
    }
  } else {
    return NULL;
  }
...
{
  if (Parser::debug.use_typedef_names && lookup_typedef_name(*this, s)) {
    return;
  }
  s = "";
  if (is_enum()) {
    s = enum_as_str(as_enum());
    return;
  }
  if (is_const()) {
    s += "const ";
...
// these are really exact or trivial matches but we need to know for arg passing
    ...
    if (t1.is_const()) {
      ...
      if(t1 == make_const(t2)) {
        return REFERENCE_MATCH;  //T& => const T&
      }
      ...
      if (t2.is_const()) {
        if (t1 == t2r) {
          return REFERENCE_MATCH;  // const T => const T&
        }
...
// if (!t2.is_ref_or_ptr()) t2.strip_const();
  if (t2.is_reference() && strip_qualifiers(t2) == t1) {
    return TRIVIAL_MATCH;
  }
...
// *fix 0.9.7 Allow for t2 => const t1&, if we can promote t2 to t1.
  ...
  Type pt = t2.promote();
  if (pt == t1) {
    return is_ref ? REF_PROMOTE_MATCH : PROMOTE_MATCH;
  }
  return NO_MATCH;
}
...
// *add 1.1.4 C is less fussy about integer conversions to enumerations...
  if (Parser::debug.c_mode && t1.is_enum() && t2.is_int()) {
    return STD_MATCH;
  }
  if (t1.is_number() && !ref_argument) {
    ...
    if (t2.is_number() || t2.is_enum()) {
      return STD_MATCH;
    }
    if (t2.is_zero()) {
      return STD_MATCH;
    }
  } else if (t2.is_pointer()) {
...
// from Lippman (2nd, 174)
  if (is_pointer() || is_reference() && ! is_variable()) {
    return *this;  //* doesn't apply!!
  }
  if (is_char() || is_short()) {
    return t_int;
  }
  if (is_short() && is_unsigned()) {
    return t_int;  // sizeof(int) > sizeof(short)
  }
  if (is_float()) {
    return t_double;
  }
...
// *add 1.1.2 Putting quotes around strings is now optional.
  ...
  }  else if (is_enum()) { // *add 1.2.6 show enumeration constants, if possible
    int val = *(int *)ptr;
    string name = as_enum()->lookup_value(val);
    out << val << ' ' << name;
  }  else if (is_bool()) {
    out << (*(int *)ptr ? "true" : "false");
  } else if (is_int()) {
    bool unsign = is_unsigned();
    if(is_char()) {
      out << '\'' << *(char *)ptr << '\'';
      if (unsign) {
        out << " (" << (unsigned_t)*(unsigned char *)ptr << ')';
      }
    } else if(is_short()) {
      if (unsign) {
        out << (unsigned_t)*(unsigned short *)ptr;
      } else {
        out << *(short *)ptr;
      }
    } else {
      if (unsign) {
        out << (unsigned_t)*(unsigned int *)ptr;
      } else {
        out << *(int *)ptr;
      }
    }
