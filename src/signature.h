...
public:
  ...
  Class *class_ptr()       const
  {
    return m_class_ptr;
  }
  void   set_class_ptr(Class *pc)
  {
    m_class_ptr = pc;
  }
  int    byte_size()       const
  ...
  {
    m_arg_types.push_back(t);
  }
  int size()       const
  {
    return m_arg_types.size();
  }
...
// this naive impl. is poss. because TypeArray _really_ is a TypeList!!
  const TypeList& type_list() const
  {
    return m_arg_types;
  }
