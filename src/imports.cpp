...
public:
  static ImportScheme *New()
  {
    return new MSImportScheme();
  }
...
{
  if (Builtin::get_dll_handle() != IMPLICIT_LINK ) {
    mCurrent = UNDECIDED;
...
void *Import::load_method_entry(Function *pfn, string& mangled_name)
{
  ...
  if (mCurrent == UNDECIDED) {
    ...
    proc = try_load_method(pfn, mCurrent);
  }
  mangled_name = mName;
  return proc;
}
