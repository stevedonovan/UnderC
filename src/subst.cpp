...
static char* stringify(char* q, char* p)
{
  bool in_quote = false;
  do {
    if (p[0] == '\"' || (in_quote && p[0] == '\\' && strchr("nrt\\\"", p[1]))) {
      *q++ = '\\';
    }
    if (p[0] == '\\' && in_quote && strchr("nrt", p[1])) {
      *q++ = *p++;
    }
    if (p[0] == '\"') {
      in_quote = !in_quote;
