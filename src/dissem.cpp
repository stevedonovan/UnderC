  ...
  while (pi != end_of_code) {
    ...
    if (opcode == CCALL || opcode == CALL || opcode == CALLD || opcode == CALLN) {
      ...
      if (opcode == CALLN) {
        ...
        pfb = PFBlock(data_ptr(rdata));
      }
      if (pfb) {
        Function::from_fun_block(pfb)->dump(std::cout);
      }
    } else if (opcode == JSWITCH) {
    ...
    } else if (opcode == TOSD || opcode == TPODS) {
    ...
    std::cout << std::endl;
    if (opcode == RET || opcode == RETI || opcode == RETD) {
      break;
    }
    pi++;
  }
