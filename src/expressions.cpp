...
PExpr bcast_op(Type t, PExpr e)
{
  if (t == e->type()) {
    ...
    if (e->type().is_plain_reference()) {
      t.make_reference();  // *fix 0.9.7 made var casts to be references!
    }
...
PExpr arith_op(int op, PExpr e1, PExpr e2)
{
  ...
  if (t1.is_number() && t2.is_number()) {
    if (t1 == t_float) {
      e1 = bcast_op(t_double, e1);
      t1 = t_double;
    }
    if (t2 == t_float) {
      e2 = bcast_op(t_double, e2);
      t2 = t_double;
    }
    for(int i = 0; i < NTYPES; i++) {
      Type tt = types[i];
      if (t1 == tt || t2 == tt) {
        if (tt == t_double && float_op[op] == 0) {
          return not_for_float(op);
        }
        if (t1 != tt) {
          e1 = bcast_op(tt, e1);
        }
    ...
    if (t1.is_pointer() && t2.is_pointer()) {
      if (op == MINUS) {
        ...
        return expr_error("Cannot add pointers");
      }
    }

    if (t1.is_pointer() && t2.is_int()) {
      return pointer_addition_op(op, e1, e2);
    } else if (t2.is_pointer() && t1.is_int()) {
      return pointer_addition_op(op, e2, e1);
    }
  }
...
// if it's an object requiring destruction, we also push the ODS!
  ...
  if (rt.as_class()->destructor()) {
    et = make_op(FCONTEXT, rt, et, 0);
  }
  args->push_front(et);
}
...
// note: will mess w/ pel!!
{
  ...
  if (!pel) {
    pel = new ExprList;  /// empty arg list
  }
  if (rt.is_object()) {
    fn_return_object(rt, pel);
  }
  pel->push_back(obj);
...
PExpr bin_op(int op, PExpr e1, PExpr e2)
{
  ...
  if (class_args) {
    ...
    if (is_object(e1)) {
      pe = e1->type().as_class()->lookup(name);
    }
...
// no entry was found, or no match possible;
  if (is_assign_op(op)) { // *fix 0.9.6 try for assignment conversions
    ...
    if (ce) {
      if (op == ASSIGN) {
        return make_op(ASSIGN, t, e1, ce);
      } else {
        return compound_assign_op(op, e1, ce);
      }
    } else {
      return cannot_convert_error(e1, e2);
    }
  } else if (op != COMMA) { // try for numerical/pointer conversions
    PExpr ce1, ce2;
    ce1 = try_convert(e1, to_number);
    ce2 = try_convert(e2, to_number);
    if (!e2 && ce1) {
      return make_op(op, ce1->type(), ce1, NULL);  // unary case!
    } else if (ce1 && ce2) {
      return arith_op(op, ce1, ce2);
    }

    if (op == PLUS || op == MINUS) { // pointer arithmetric
      if (ce2) {
        ce1 = try_convert(e1, to_pointer);
      }
      if (ce1) {
        ce2 = try_convert(e2, to_pointer);
      }
      if (ce1 && ce2) {
        return arith_op(op, ce1, ce2);
      }
  ...
  if (class_args) {
    ...
    if (op == EQUAL) {
      return constant_op(t_int, 0);
    } else {
      return function_error();
    }
  } else {
    return make_op(op, e1->type(), e1, e2);
  }
}
...
// *change 1.1.0 This is handled by the builtin delete_ex, unless delete is overloaded.
  ...
  if (pc != NULL) {
    if (pc->destructor()) {
      ...
      if (is_arr) {
        ed = vector_context_op(ed);
      } else {
        ed = dynamic_context_op(ed);
      }
      if (overloaded && overalloc) {
        ed = add_const_op(t, -sizeof(void *), ed);
      }
      ed = append_op(e1, ed);
    } else {
      ed = e1;
...
// call the constructor, passing the arguments
  ...
  if (pec == NULL) {
    return expr_error("cannot create constructor");
  }
...
static PExpr array_init_op(PEntry pe, PExprList pel)
{
  ...
  for(int i = 0; eli != pel->end(); ++eli, ++i) {
    PEntry aie = new Entry;
    *aie = *pe;	   // construct an Array Item Entry
    ...
    if (i > 0) {
      arr_expr = append_op(arr_expr, assign_expr, true);  // i.e normal COMMA op!
    } else {
      arr_expr = assign_expr;
    }
  }
  return arr_expr;
}
...
static PExpr struct_init_op(PExpr obj, PClass pc, PExprList pel)
{
  ...
  if (fields.size() != pel->size()) {
    return expr_error("init list wrong size for this struct");
  }
  for(ei = fields.begin(), eli = pel->begin(); ei != fields.end(); ++ei, ++eli) {
    assign_expr = init_op(entry_op(*ei), Parser::array_size(*ei), *eli, NULL, 0);
    if (assign_expr->is_nil()) {
      return assign_expr;  // couldn't assign to this entry!
    }
    if (ei != fields.begin()) {
      init_expr = append_op(init_expr, assign_expr, true);
    } else {
...
// Only one of er and pel is non-NULL!
  ...
  if (t.is_class() && !is_ref && (is_non_init_array || !t.is_pointer())) {
    PClass pc = t.as_class();
    if (brace_init) {
      return struct_init_op(el, pc, pel);  // initialized struct!
    }
    if (t.as_class()->has_constructors()) { // an object or an uninitialized array of objects...
      if (er) {
        pel = expr_list(er);  // object initialized w/ assign syntax
      }
      if (!pel) {
        pel = expr_list();  // i.e, call default constructor!
      }
      PExpr ec = construct_op(pc, pel);
      if (ec->is_nil()) {
        if (pel->size() == 0) {
          return expr_error("no default constructor");
        }
        return expr_error("cannot find constructor");
      }
      // either object (initialized or not), or uninitialized array.
      if (! is_non_init_array) {
        return construct_context_op(ec, el, ctype);
      } else {
        return vector_context_op(ec, el);
      }
    } else
      // *fix 1.2.6 plain simple structs are just initialized with a copy operation
      if (er != NULL) {
        return make_op(COPY_BLOCK, t, el, er);
      } else {
        return NULL;  // no operation..
      }
  } else { // reference or simple initialization
    if (pel) {
      er = pel->front();  // scalar init. w/ object syntax
    }
    if (!er) {
      return expr_error("bad init");
    }
    if (is_ref) {
      return init_ref_op(el, er);  // init. reference
    } else if (is_array) {
...
// *add 1.2.0L special case implemented
      ...
      return assign_op(el, er, false);  // simple initialization
    }
  }
}
...
// *NOTE* Do we do new[] yet?
  ...
  const char *new_fn;
  if (is_scalar) {
    new_fn =  overload ? "new" :   (overalloc ? "_new_ex" : "_new");
  } else {
    new_fn =  overload ? "new[]" : (overalloc ? "_new_vect_ex" : "_new_vect");
  }
  PExpr er, ez = constant_op(t_int, t.size());
  t.incr_pointer();
// allocating the memory
  if (is_scalar) {
    er = function_call(new_fn, expr_list(ez));
  } else {
    er = function_call(new_fn, expr_list(e, ez));
  }
...
// a result of T* needs construction if T::T() exists;  but T** etc doesn't!
  if (pc && t.pointer_depth() == 1 && pc->has_constructors()) {
    ...
    if (is_scalar) {
      ec = dynamic_context_op(ector);
    } else {
      ec = vector_context_op(ector);
    }
    // NB to skip first word if we're putting a VMT there!
    if (overalloc && overload) {
      er = add_const_op(t, sizeof(void *), er);
    }
...
// *fix 0.9.6 We didn't check to see if the _RHS_ was an object..
  if (is_object(t) || is_object(e2->type())) {
    if (is_object(t) && t.as_class()->simple_struct()) {
      return make_op(COPY_BLOCK, t, e1, e2);
    } else {
      return bin_op(ASSIGN, e1, e2);
    }
  } else {
...
// I'm sure this is going to cause grief elsewhere!
  ...
  if (t.is_object() && (ce = convert_type_op(t_int, e1)) != NULL) {
    return ce;
  } else {
    return e1;
  }
...
PExpr compound_assign_op(int op, PExpr e1, PExpr e2)
{
  if (is_object(e1->type()) || is_object(e2->type()) ) {
    ...
    return assign_op(e1, arith_op(assign_equiv_op[op], e1, e2));
  }
}
...
PEntry add_temporary(Type t, PExpr e, bool always_construct, bool dont_call_dtor)
{
  ...
  if (context == temp_context()) {
    context = temp_context()->parent_context();
  }
  ...
  if(rt.is_reference()) {
    ...
    if (rt.as_class()->has_constructors()) {
      ...
      if (ctor->is_nil()) {
        ...
        return construct_context_op(ctor, re);
      }
    } else {
      return assign_op(re, e, false);
    }
  } else {
...
PExpr deref_op(PExpr e1, bool do_overload)
{
  ...
  if (t.is_pointer()) {
    ...
    return unary_op(DEREF, e1);
  } else {
    return expr_error("Was not a pointer");
  }
}
...
// *fix 1.1.2 This typecast is necessary even if we're not an entry!
    ...
    if (t.is_function()) {
      t = Type(unique_signature(t.as_signature()));
    }
    t.incr_pointer();
    return make_op(ADDR, t, e1);
  } else if (do_overload) {
    return unary_op(ADDR, e1);
  } else {
    return NULL;  /// expr_error("Not an lvalue"); *now caller is responsible*
  }
}
...
// this JUST does integers
  if((op == UMINUS || op == BIN_NOT) && e1->is_entry() && e1->type() == t_const_int) {
...
PExpr typecast_op(int which, Type t, PExpr e)
{
  ...
  if (which == DYNAMIC_CAST) { // *add 0.9.5 dynamic cast
    if (! t.is_class() && t != t_void_ptr) {
      return expr_error("dynamic_cast: must be class or void *");
    }
    if (!type_has_VMT(t) || !type_has_VMT(e->type())) {
      return expr_error("dynamic_cast: class must have virtual methods");
    }
...
PExpr function_cast_op(Type t, PExprList pel)
{
  if (t.is_class() && !t.is_ref_or_ptr()) {
    ...
    if (pel->size() != 1) {
      return expr_error("typecast needs one argument");
    }
...
// (cd easily be constant, but what the hell...
  ...
  if (t.is_reference()) { // NOTE(3) hack!!
    if (t.is_const()) {
      ...
      return not_for_float(op);  //*NOTE* this isn't true!
    }
    //else return expr_error("Que?");
  }
//else
  if (t.is_class()) {
    return bin_op(op, e, extra);
  } else {
    return must_be_lvalue(op);
  }
}
...
// *fix 1.1.3 will also try to use ctors to construct a type!
  ...
  if (try_conversions_from && t.is_class()) {
    ...
    if (pf != NULL) {
      return construct_temporary_op(t, e);
    }
  }
...
// An empty function expression allows expr lists to be expressions
  if (!e) {
    return new Expr(FUNCTION, t_void, NULL, args);
  }
  if(e->op() == NIL) {
    return e;  // error in function expression
  }
  if (!args) {
    args = new ExprList;  /// empty arg list
  }
...
// *add 1.1.0 support for pre-constructing objects when passed by value
    ...
    for (eli = args->begin(); i < n_sig; ++eli, ++mli, ++i) {
      ...
      case REF_PROMOTE_MATCH:
        if (et.is_variable()) {
          et.strip_qualifiers();  // *hack 0.9.7 trouble w/ is_object()!
        }
...
// *add 1.2.3 Suppressed errors (like when we are looking at operators) are saved for later...
  ...
  if (returns_object && ! true_value_return) {
    fn_return_object(rt, args);
  }
...
// *fix 0.9.6 The DCALL function type _forces_ a function to be directly called, virtual or not
  ...
  if (was_method) {
    args->push_back(e->arg1());  // append obj ptr to arg list
  }
...
// finally, if it was a class template method then instantiate it (if not already)
  if (!was_fn_ptr && pfe->get_template() && pfe->get_template()->is_method())
    if (!pfe->get_template()->instantiate_method(fn)) {
      return expr_error("instantiation failed");
    }

  if (returns_object && true_value_return) {
    return fn_return_object_as_value(rt, er);
  } else {
    return er;
  }
}
...
PExpr selection_op(PExpr e, char *name, bool is_ptr, bool is_member_ptr)
{
  ...
  if (!t.is_class()) {
    return expr_error("Not a class object or pointer");
  }
  PClass pc = t.as_class();
  PEntry pe;
  bool not_found;
  ...
  if (is_ptr && !was_pointer) { // -> may be overloaded...
    ...
    if (efn->op() != ARROW) { // was overloaded!
      Type rtype = efn->type();
      if (!rtype.is_class()) {
        return expr_error("operator-> should return a class");
      }
  ...
  if (not_found) {
    return expr_not_found_error(name, pc);
  }
  if (!was_pointer && is_ptr) {
    return expr_error("Must be a pointer");
  } else if (was_pointer && !is_ptr) {
    return expr_error("Cannot be an object pointer");
  }
  if (was_pointer) {
    e = deref_op(e, false);
  }
...
// arrays are _not_variables!
  Type t = pe->type;
  if (!t.is_reference()) {
    if (pe->name != "" && Parser::array_size(pe) == 1) {
      t.make_variable();
    }
...
// make a deep copy of an expression!
{
  ...
  // for all other operators, can treat the arguments as expressions
    ...
    er = new Expr(op, t, clone(e->arg1()), clone(e->arg2()));
  }
  return er;
}
...
void dump(std::ostream& os, PExpr e)
{
  ...
  if (e->is_function() || e->is_expr()) {
    ...
    os << "<nil>";
  } else if (! e->is_entry()) { // i.e. a binary or unary op!
    os << " (";
    if (! e->is_bcast()) {
      os << e->name();
    } else {
